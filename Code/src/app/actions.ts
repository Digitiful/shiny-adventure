...
function getAdminFirestoreInstance() {
    ...
    if (!adminApp) {
        throw new Error("UNAUTHORIZED: Master Signal (Admin SDK) not initialized.");
    }
...
export async function handleBookingForm(
  ...
  if (!validatedFields.success) {
    ...
    return { message: firstError, status: "error" };
  }
  ...
  try {
    ...
    submissionDate.setHours(hours, parseInt(timeParts[1] || "0"));
...
export async function handleTerminalQuery(query: string, userName?: string | null, history?: HistoryLine[]) {
  try {
    ...
    console.error("AI Terminal query failed:", error);
    return { answer: "SIGNAL INTERFERENCE. ATTEMPTING TO RE-STABILIZE CORE." };
  }
}
...
export async function handleGeneralQuery(query: string, history?: HistoryLine[]) {
  try {
    ...
    return { answer: "Signal lost. Systems recalibrating..." };
  }
}
...
export async function handleAddService(
    ...
    try {
        ...
        return { status: 'error', message: 'Failed to add service. Please try again.' };
    }
}
...
export async function handleDeleteService(
    ...
    if (!id) return { status: 'error', message: 'Invalid service ID.' };
    ...
    try {
        ...
        return { status: 'error', message: 'Failed to delete service. Please try again.' };
    }
}
...
export async function handleDeleteTask(formData: FormData): Promise<{ status: 'success' | 'error'; message: string; }> {
    ...
    try {
        ...
        return { status: 'error', message: 'Failed to delete task. Please try again.' };
    }
}
