...
import { Plus, LayoutGrid, Phone, Library, Archive, CaseUpper, FileVideo, Loader2, Activity as ActivityIcon, ShieldCheck, Zap } from "lucide-react";
    ...
    <div className="flex flex-col min-h-screen bg-background relative overflow-hidden">
        ...
        <div className="container mx-auto px-4">
            ...
            <div className="flex items-center gap-4">
              ...
              <div>
                <h1 className="font-headline text-4xl font-black tracking-tighter text-primary uppercase italic">
                  Alien Warehouse <span className="text-foreground/50 not-italic font-light">//</span> OPS
                </h1>
                <p className="text-[10px] font-code text-muted-foreground uppercase tracking-[0.2em] flex items-center gap-2">
                  <span className="h-1.5 w-1.5 rounded-full bg-green-500 animate-pulse"></span>
                  System Status: Deliberate // Missions Active
                </p>
              </div>
            </div>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-4 gap-6 mb-8">
             <Card className="bg-primary/5 border-primary/10">
                ...
                <CardContent>
                    ...
                    <p className="text-[9px] text-muted-foreground uppercase mt-1">Artifacts Secured</p>
                </CardContent>
             </Card>
          ...
          <Tabs defaultValue="hub" className="w-full">
            <TabsList className="grid w-full grid-cols-2 sm:grid-cols-5 md:w-auto mb-8 bg-muted/20 border border-primary/10">
              <TabsTrigger value="hub" className="data-[state=active]:bg-primary/20"><CaseUpper className="mr-2 h-4 w-4"/>Project Hub</TabsTrigger>
              ...
              <TabsTrigger value="knowledge" className="data-[state=active]:bg-primary/20"><Library className="mr-2 h-4 w-4"/>Knowledge Base</TabsTrigger>
              <TabsTrigger value="analytics" className="data-[state=active]:bg-primary/20"><FileVideo className="mr-2 h-4 w-4"/>Analytics</TabsTrigger>
            </TabsList>
               ...
               <Card className="bg-card/30 backdrop-blur-md border-primary/10 shadow-2xl">
                <CardHeader className="border-b border-primary/5 flex flex-row items-center justify-between">
                  <div>
                    ...
                    <CardDescription className="text-[10px] font-code uppercase">Drag artifacts to update operational status.</CardDescription>
                  </div>
                ...
                <CardContent className="pt-6">
                  ...
                  >
                    <div className="hidden md:grid md:grid-cols-2 lg:grid-cols-4 gap-6">
                      ...
                      <KanbanColumn title="In Development" status="in-progress" tasks={inProgressTasks} isLoading={isLoadingTasks} onAddTaskClick={() => openAddTaskDialog('in-progress')} onTaskClick={openEditTaskDialog} />
                      <KanbanColumn title="Deployed" status="done" tasks={doneTasks} isLoading={isLoadingTasks} onAddTaskClick={() => openAddTaskDialog('done')} onTaskClick={openEditTaskDialog} />
                      <KanbanColumn title="Archived" status="archived" tasks={archivedTasks} isLoading={isLoadingTasks} onAddTaskClick={() => openAddTaskDialog('archived')} onTaskClick={openEditTaskDialog} icon={Archive} isArchivedColumn/>
                    </div>
                        ...
                        <Tabs defaultValue="todo" className="w-full">
                          ...
                          <TabsContent value="todo">
                           ...
                           <TabsContent value="in-progress">
                              <KanbanColumn title="In Development" status="in-progress" tasks={inProgressTasks} isLoading={isLoadingTasks} onAddTaskClick={() => openAddTaskDialog('in-progress')} onTaskClick={openEditTaskDialog} hideTitle />
                          </TabsContent>
                           <TabsContent value="done">
                              <KanbanColumn title="Deployed" status="done" tasks={doneTasks} isLoading={isLoadingTasks} onAddTaskClick={() => openAddTaskDialog('done')} onTaskClick={openEditTaskDialog} hideTitle />
                          </TabsContent>
                          <TabsContent value="archived">
                              <KanbanColumn title="Archived" status="archived" tasks={archivedTasks} isLoading={isLoadingTasks} onAddTaskClick={() => openAddTaskDialog('archived')} onTaskClick={openEditTaskDialog} icon={Archive} hideTitle isArchivedColumn />
                          </TabsContent>
                        </Tabs>
                     </div>
            ...
            <TabsContent value="knowledge">
              <Card className="bg-card/30 backdrop-blur-md border-primary/10">
                <CardHeader>
                  ...
                  <CardDescription className="text-xs">Resources for the Alien Warehouse protocol.</CardDescription>
                </CardHeader>
                <CardContent>
      ...
      <Dialog open={isAddFormOpen} onOpenChange={setIsAddFormOpen}>
        <DialogContent className="bg-background/95 border-primary/20">
          <DialogHeader>
            <DialogTitle className="text-primary flex items-center gap-2"><Zap className="h-5 w-5" /> Initiate Artifact</DialogTitle>
            <DialogDescription className="font-code text-[10px] uppercase">Define new mission parameters for Alien Warehouse.</DialogDescription>
          </DialogHeader>
          <AddTask 
      ...
      <Dialog open={isEditFormOpen} onOpenChange={setIsEditFormOpen}>
        <DialogContent className="sm:max-w-[700px] bg-background/95 border-primary/20">
          <DialogHeader>
            <DialogTitle className="text-primary">Recalibrate Artifact</DialogTitle>
            <DialogDescription className="font-code text-[10px] uppercase">Modifying mission ID: {selectedTask?.id}</DialogDescription>
          </DialogHeader>
          {selectedTask && (
            <EditTask 
              task={selectedTask}
