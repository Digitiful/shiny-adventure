...
const SYSTEM_LOGS = [
    "ROUTING PACKET THROUGH MONTREAL-NY EDGE...",
    "VERIFYING ANTIGRAVITY SHIELD INTEGRITY...",
    "ENCRYPTING SESSION DATA WITH AES-256...",
    "SYNCING WITH ANTIGRAVITY VAULT...",
    "PULLING LATEST HEURISTICS FROM GEMINI-2.5...",
    "UPDATING OPERATOR CLEARANCE...",
    "OPTIMIZING THROUGHPUT FOR DELIBERATE EXECUTION...",
...
export function BookingTerminal() {
    ...
    const [lines, setLines] = useState<Line[]>([
        ...
        { type: 'system', text: "AI TERMINAL 3.0 // NODE: OPERATIONAL" },
        { type: 'ai', text: `Transmission received. ${user ? `Welcome back, ${user.displayName || 'Operator'}.` : "I am Digi."} How shall we optimize your system today?` },
        { type: 'system', text: "Commands: 'book', 'spec', 'stack', 'uptime', 'signal', 'history', 'clear', 'exit'" },
    ]);
    ...
    const resetTerminal = () => {
        setLines([
            ...
            { type: 'system', text: "Enter 'spec' for system DNA or 'book' to initiate extraction." },
        ]);
        setHistory([]);
        setCommandHistory([]);
        setHistoryIndex(-1);
    ...
    const handleBookingInput = async (val: string) => {
        ...
        if (bookingStep === 'day') {
            ...
            setBookingStep('time');
            setLines(prev => [...prev, 
                { type: 'system', text: "> STEP 02: SELECT EXTRACTION WINDOW" },
                { type: 'ai', text: "Select a slot: [A] 09:00 EST | [B] 13:00 EST | [C] 17:00 EST" }
            ]);
        } 
            ...
            setBookingStep('name');
            setLines(prev => [...prev, 
                { type: 'system', text: "> STEP 03: AUTHENTICATE OPERATOR" },
                { type: 'ai', text: "What is your identified name?" }
            ]);
        }
            ...
            setLines(prev => [...prev, 
                { type: 'system', text: "> STEP 04: PROVIDE CONTACT SIGNAL" },
                { type: 'ai', text: "Enter your secure email address for confirmation." }
            ]);
        }
            ...
            setLines(prev => [...prev, 
                { type: 'system', text: "> STEP 05: MISSION OBJECTIVE" },
                { type: 'ai', text: "Briefly describe the system challenge or project requirements." }
            ]);
        }
            ...
            setBookingStep('none');
            setIsThinking(true);
            setLines(prev => [...prev, { type: 'system', text: "> TRANSMITTING EXTRACTION PACKET..." }]);
            ...
            try {
                ...
                if (result.status === 'success') {
                    ...
                    setLines(prev => [...prev, { type: 'ai', text: "Extraction confirmed. Stand by for contact on your provided signal." }]);
                } else {
                    setLines(prev => [...prev, { type: 'error', text: `> TRANSMISSION FAILED: ${result.message}` }]);
                }
            } catch (err) {
                setLines(prev => [...prev, { type: 'error', text: "> CRITICAL ERROR: ANTIGRAVITY FIREWALL ACTIVE. TRY AGAIN LATER." }]);
            } finally {
                setIsThinking(false);
            }
        }
    ...
    const handleInput = async (e: React.KeyboardEvent<HTMLInputElement>) => {
        ...
        if (command === 'book') {
            setBookingStep('day');
            setLines(prev => [...prev, 
                { type: 'system', text: "> INITIATING EXTRACTION PROTOCOL v3.0..." },
                { type: 'system', text: "> STEP 01: SPECIFY TARGET DATE" },
                { type: 'ai', text: "Which day shall we initiate the extraction? (e.g. Next Monday, April 12th)" }
            ]);
...
> Color: #8338EC, #BA89FF, #F72585, #83B3FF, #3A86FE
> DNA_HASH: [ANOMALY_PURPLE]
> UPTIME: 99.99% [OK]
...
> DATA: Antigravity Vault
> SECURITY: Antigravity Shield
...
> LATENCY: ${latency}MS
> BANDWIDTH: OPTIMIZED
> OPERATOR: ${user ? user.displayName?.toUpperCase() : 'UNKNOWN'} // ACTIVE`;
            setLines(prev => [...prev, { type: 'system', text: signalText }]);
        } else if (command === 'history') {
            const historyText = commandHistory.map((cmd, i) => `[${i+1}] ${cmd}`).join('\n');
            setLines(prev => [...prev, { type: 'system', text: `> COMMAND LOG:\n${historyText || "NO COMMANDS RECORDED."}` }]);
            ...
            try {
                ...
                setLines(prev => [...prev, { type: 'error', text: "SIGNAL INTERFERENCE. AI ENGINE RECALIBRATING." }]);
            } finally {
                setIsThinking(false);
            }
        }
            ...
            <div className="container mx-auto px-4">
                <div className="max-w-3xl mx-auto text-center mb-12">
                    ...
                    <p className="mt-4 text-lg text-muted-foreground font-medium">
                        Initiate a direct link with Digi. for system diagnostics and technical specs.
                    </p>
                </div>
                    ...
                    <div ref={scrollAreaRef} className="p-4 h-[550px] overflow-y-auto scrollbar-thin scrollbar-thumb-primary/20 bg-black/40">
                        ...
                        {isThinking && (
                             <div className="flex items-start text-muted-foreground animate-pulse">
                                ...
                                <p className="text-[11px] uppercase">Synthesizing response from core...</p>
                            </div>
                        )}
                        ...
                        <div className="flex items-center mt-2 border-l-2 border-primary pl-2 bg-primary/5">
                            ...
                            <input
                                ...
                                placeholder={bookingStep !== 'none' ? "Transmit answer..." : "Enter command or query..."}
                            />
                        </div>
